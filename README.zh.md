...
键盘、浏览器控制、Computer Use **仍未实现**。

- `/nodes` · `/node_status` —— 列出已知执行节点、能力以及动态的 online/offline 状态。
- `/computer_status` —— 显示 Computer Use 状态（当 desktop agent 运行时为 online）。
- 自然语言：`我的节点`、`机器状态`、`主机状态`、
  `MacBook 在线吗`、`desktop node`、`nodes status`、
  `computer use status`。带桌面控制目标的人话（`帮我在 Mac 上打开
  Xcode`）会走确定性 stub。截屏 observe 类短语（`take a screenshot
  on my desktop`）走 `desktop.screenshot.status`（仅状态，不远程截屏）。
...
* **Helper**：在 `capture-your-screen` 仓库构建 `capture-screen-helper`，配置 `CONVEYOR_DESKTOP_SCREENSHOT_HELPER`。
* **本地截屏**：`.venv/bin/python desktop_agent.py --observe-once`
* **状态**：`/desktop_screenshot_status` 或截屏类自然语言。
* 截图默认保存在 `CODEX_MEMORY_ROOT/desktop/screenshots/`，不上传。
* macOS 需要 Screen Recording 权限。

> **Computer Use 控制仍未实现。** P5.2 observe 不会移动鼠标、键盘输入或调用 Gemini Computer Use。详见 `docs/desktop_screenshot_observe.md`。
...
- **未实现 Computer Use。** 屏幕截图、桌面控制、鼠标/键盘模拟操作以及 Gemini Computer Use 调用均**未实现**。
